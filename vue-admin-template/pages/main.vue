<template>
  <el-container id="main">
    ...
    <el-container class="content">
      <div class="aside-nav-bg slow-motion" :class="!!asideNavOpen ? 'open' : ''"
           @click.self="asideNavOpen = !asideNavOpen"></div>
      <el-aside class="aside-nav slow-motion" :class="[asideClass, !!asideNavOpen ? 'open' : '']">
...
export default {
  ...
  setup(prop, ctx) {
    ...
    const childView = computed(() => {
      ...
      // if (!childNav[ci].has && config.navServe) continue;
      if (
    ...
    function logout() {
      const {loading, error, data, run} = useRequest(userApi.logout);
      watch(data, (val) => {
    ...
    let isWrapBreadcrumb = ref(false);

    let asideNavOpen = ref(false);
    ...
    // password
    let editPassViewDialogtitle = ref('修改密码');
    let editPassViewDialogVisible = ref(false);
